...
TypeScript runs natively via Node's `--experimental-strip-types`. `tsc --build` compiles `src/` to `dist/`, JavaScript and `.d.ts` declarations, which the CLI and the tests run.
...
- **`src/markdown/`** — compiler. Owns markdown parsing, code highlighting (Shiki), mermaid rendering (mermaid, drawn in headless Chromium through Playwright), and everything else "markdown-flavored." Normalizes the deck spec so the engine only sees text runs, tables, and resolved image paths (a deck names every image by a path relative to itself).
