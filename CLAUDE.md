...
- **`skills/create-theme/`** — the theme-authoring Agent Skill. **`theme-package/`** — the deck-authoring SKILL.md that `tycoslide package` puts in every skill zip. **`docs/markdown.md`** — the deck language reference. **`docs/theme.md`** — the `theme.json` reference.
