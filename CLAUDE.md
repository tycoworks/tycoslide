...
- **`skills/create-theme/`** — the theme-authoring Agent Skill (SKILL.md, references). **`theme-package/`** — the deck-authoring SKILL.md that `tycoslide package` copies into every theme. **`docs/syntax.md`** — the deck language reference, which `package` copies too.
