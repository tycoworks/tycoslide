// Diagram Tests
// Tests for DiagramBuilder data structure creation
// Migrated from diagram.test.ts.old - now tests data structure, not mermaid output

import { describe, test } from 'node:test';
import * as assert from 'node:assert';
import { diagram, DiagramBuilder, DIAGRAM_DIRECTION, NODE_STYLE } from '../src/components/diagram.js';
import { NODE_TYPE } from '../src/core/nodes.js';

describe('DiagramBuilder', () => {

  // ------------------------------------------
  // Node Type and Direction
  // ------------------------------------------
  test('creates DiagramNode with correct type', () => {
    const d = diagram(DIAGRAM_DIRECTION.LEFT_TO_RIGHT);
    assert.strictEqual(d.type, NODE_TYPE.DIAGRAM);
  });

  test('stores direction correctly', () => {
    const d = diagram(DIAGRAM_DIRECTION.TOP_TO_BOTTOM);
    assert.strictEqual(d.direction, 'TB');
  });

  test('defaults to LEFT_TO_RIGHT direction', () => {
    const d = diagram();
    assert.strictEqual(d.direction, 'LR');
  });
  ...
  test('rect() creates node with rect shape', () => {
    ...
    assert.strictEqual(d.nodes.length, 1);
    assert.deepStrictEqual(d.nodes[0], { id: 'A', label: 'Start', shape: 'rect' });
  });
  ...
  test('rect() uses id as label when label not provided', () => {
    ...
    assert.strictEqual(d.nodes[0].label, 'MyNode');
  });
  ...
  test('cylinder() creates node with cylinder shape', () => {
    ...
    assert.strictEqual(d.nodes[0].shape, 'cylinder');
  });
  ...
  test('hexagon() creates node with hexagon shape', () => {
    ...
    assert.strictEqual(d.nodes[0].shape, 'hexagon');
  });
  ...
  test('edge() creates edge between two nodes', () => {
    ...
    assert.deepStrictEqual(d.edges[0], { from: ['A'], to: ['B'], label: undefined });
  });
  ...
  test('edge() with label stores label', () => {
    ...
    assert.strictEqual(d.edges[0].label, 'connects');
  });
  ...
  test('edge() with multiple targets creates multi-target edge', () => {
    ...
    assert.deepStrictEqual(d.edges[0].from, ['A']);
    assert.deepStrictEqual(d.edges[0].to, ['B', 'C']);
  });
  ...
  test('edge() with multiple sources creates multi-source edge', () => {
    ...
    assert.deepStrictEqual(d.edges[0].from, ['A', 'B']);
    assert.deepStrictEqual(d.edges[0].to, ['C']);
  });
  ...
  test('subgraph() creates subgraph with nodes', () => {
    ...
    assert.strictEqual(d.subgraphs.length, 1);
    assert.strictEqual(d.subgraphs[0].id, 'Sources');
    assert.deepStrictEqual(d.subgraphs[0].nodeIds, ['DB']);
  });
  ...
  test('subgraph() with custom label', () => {
    ...
    assert.strictEqual(d.subgraphs[0].label, 'Processing');
  });
  ...
  test('subgraph() with direction', () => {
    ...
    assert.strictEqual(d.subgraphs[0].direction, 'TB');
  });
  ...
  test('subgraph() with multiple nodes', () => {
    ...
    assert.deepStrictEqual(d.subgraphs[0].nodeIds, ['A', 'B', 'C']);
  });
  ...
  test('class() applies style to single node', () => {
    ...
    assert.strictEqual(d.classes.length, 1);
    assert.strictEqual(d.classes[0].nodeId, 'A');
    assert.strictEqual(d.classes[0].style, NODE_STYLE.PRIMARY);
  });
  ...
  test('class() applies style to multiple nodes', () => {
    ...
    assert.strictEqual(d.classes.length, 2);
    assert.strictEqual(d.classes[0].nodeId, 'A');
    assert.strictEqual(d.classes[1].nodeId, 'B');
  });
  ...
  test('multiple class() calls accumulate', () => {
    ...
    assert.strictEqual(d.classes.length, 3);
    assert.strictEqual(d.classes[0].style, NODE_STYLE.PRIMARY);
    assert.strictEqual(d.classes[1].style, NODE_STYLE.BACKGROUND);
    assert.strictEqual(d.classes[2].style, NODE_STYLE.BACKGROUND);
  });
  ...
  test('scale defaults to undefined', () => {
    const d = diagram();
    assert.strictEqual(d.scale, undefined);
  });
  ...
  test('full demo-style diagram has correct structure', () => {
    ...
    // Verify nodes
    assert.strictEqual(d.nodes.length, 6);
    assert.strictEqual(d.nodes[0].id, 'DB');
    assert.strictEqual(d.nodes[0].shape, 'cylinder');

    // Verify subgraphs
    assert.strictEqual(d.subgraphs.length, 3);
    assert.strictEqual(d.subgraphs[0].id, 'Sources');
    assert.strictEqual(d.subgraphs[1].id, 'Core');
    assert.strictEqual(d.subgraphs[1].label, 'Processing');
    assert.strictEqual(d.subgraphs[1].direction, 'LR');
    assert.deepStrictEqual(d.subgraphs[1].nodeIds, ['T', 'I', 'M', 'P']);

    // Verify edges
    assert.strictEqual(d.edges.length, 3);
    assert.deepStrictEqual(d.edges[0].to, ['T', 'I', 'M']);
    assert.deepStrictEqual(d.edges[1].from, ['T', 'I', 'M']);
    assert.strictEqual(d.edges[2].label, 'STREAM');

    // Verify classes
    assert.strictEqual(d.classes.length, 6); // 4 PRIMARY + 2 BACKGROUND
  });
});
