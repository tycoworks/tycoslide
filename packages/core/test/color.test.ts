...
import { hexToRgba, bgColor, stripHash } from '../src/utils/color.js';

describe('stripHash', () => {
  test('strips leading # from hex color', () => {
    assert.strictEqual(stripHash('#FF0000'), 'FF0000');
  });
  ...
  test('strips # from lowercase hex', () => {
    assert.strictEqual(stripHash('#aabbcc'), 'aabbcc');
  });

  test('does not strip # from middle of string', () => {
    assert.strictEqual(stripHash('FF#0000'), 'FF#0000');
  });
});
