...
describe("HTML Measurement Generation", () => {
  ...
  describe("Bullet rendering", () => {
    ...
    test("multi-run bullet renders all runs in one li", async () => {
      ...
      const node = textNode([{ text: "Bold:", bold: true, bullet: true }, { text: " plain text" }]);
      const { html } = await genHTML(node, bounds);
    ...
    test("multi-run ordered bullet item renders all runs in one li", async () => {
      const node = textNode([{ text: "Bold:", bold: true, bullet: { type: "number" } }, { text: " detail text" }]);
      const { html } = await genHTML(node, bounds);
      const liMatch = html.match(/<li[^>]*>(.*?)<\/li>/);
      assert.ok(liMatch, "Should have a <li> element");
      const liContent = liMatch![1];
  ...
  describe("Vertical alignment (safe overflow protection)", () => {
    test("column with vAlign MIDDLE generates safe center", async () => {
      ...
      const { html } = await genHTML(node, bounds);
      assert.ok(
        html.includes("safe center"),
        'vAlign MIDDLE should use "safe center" — prevents content above parent in measurements',
      );
    });
  ...
  describe("Cross-axis stretch (containers fill parent height in rows)", () => {
    ...
    test("container in row omits height (stretch handles cross-axis fill)", async () => {
      ...
      assert.ok(columnMatch, "Should find the nested column div");
      assert.ok(
        !columnMatch![1].includes("height:100%"),
        "Column in row should not have height:100% (stretch handles it)",
      );
    });
    ...
    test("stack in row omits height (stretch handles card equal height)", async () => {
      ...
      assert.ok(
        !stackMatch![1].includes("height:100%"),
        "Stack in row should not have height:100% (stretch handles it)",
      );
    });
    ...
    test("stack child (grid item) is flex container filling the grid cell", async () => {
      ...
      assert.ok(
        ...
        "Stack child should be flex container",
      );
      assert.ok(
        !html.includes("overflow:hidden"),
        "Stack child should NOT have overflow:hidden (compression is in grid template)",
      );
    });
  ...
  describe("Compressibility rules", () => {
    ...
    test("image in definite-height row uses natural aspect-ratio width (flex: 0 1 auto)", async () => {
      ...
      assert.ok(imageMatch, "Should find the image div");
      assert.ok(
        imageMatch![1].includes("flex:0 1 auto"),
        "Image in definite-height row should use natural width (flex:0 1 auto)",
      );
    ...
    test("image in SIZE.FILL row uses natural aspect-ratio width (flex: 0 1 auto)", async () => {
      ...
      assert.ok(
        imageMatch![1].includes("flex:0 1 auto"),
        "Image in SIZE.FILL row should use natural width (flex:0 1 auto)",
      );
    });
  ...
  describe("heightIsConstrained propagation", () => {
    ...
    test("FILL chain propagates constraint to image (min-height:0)", async () => {
      ...
      const node = colNode(
        { height: SIZE.FILL },
        colNode({ height: SIZE.FILL }, colNode({ height: SIZE.FILL }, imageNode(testImage))),
      );
      const { html } = await genHTML(node, bounds);
    ...
    test("stack preserves min-content grid template even in constrained context", async () => {
      ...
      assert.ok(
        stackMatch![1].includes("min-content"),
        "Stack should use min-content grid template (preserves padding)",
      );
      assert.ok(
        !stackMatch![1].includes("minmax(0"),
        "Stack should NOT use minmax(0, ...) (would allow crushing content)",
      );
    });
    ...
    test("row passes constraint through to images in nested columns", async () => {
      ...
      const node = colNode(
        { height: SIZE.FILL },
        rowNode({ height: SIZE.FILL }, colNode({ height: SIZE.FILL }, imageNode("./test/fixtures/test.png"))),
      );
      const { html } = await genHTML(node, bounds);
      const imageMatch = html.match(/data-node-id="node-4"[^>]*style="([^"]*)"/);
      assert.ok(imageMatch, "Should find the image (node-4)");
      assert.ok(
        imageMatch![1].includes("min-height:0"),
        "Row should pass constraint through — image gets min-height:0",
      );
    ...
    test("HUG row breaks chain for its children", async () => {
      ...
      const node = colNode({ height: SIZE.FILL }, rowNode(colNode({ height: SIZE.FILL }, textNode("inside HUG row"))));
      const { html } = await genHTML(node, bounds);
      const innerCol = html.match(/data-node-id="node-3"[^>]*style="([^"]*)"/);
      ...
      assert.ok(
        !innerCol![1].includes("min-height:0"),
        "HUG row should break chain — inner column should NOT get min-height:0",
      );
    });
    ...
    test("image in constrained column gets min-height:0 through row", async () => {
      ...
      const node = colNode(
        { height: SIZE.FILL },
        rowNode({ height: SIZE.FILL }, colNode({ height: SIZE.FILL }, imageNode(testImage))),
      );
      const { html } = await genHTML(node, bounds);
      const imgMatch = html.match(/data-node-id="node-4"[^>]*style="([^"]*)"/);
      assert.ok(imgMatch, "Should find image (node-4)");
      assert.ok(
        imgMatch![1].includes("min-height:0"),
        "Image in column inside constrained row should get min-height:0",
      );
    });
  });
  ...
  describe("SoftBreak rendering", () => {
    test("softBreak renders as br tag (not paragraph spacing)", async () => {
      const node = textNode([{ text: "Line one" }, { text: "Line two", softBreak: true }]);
      const { html } = await genHTML(node, bounds);
  ...
  describe("Table headerCol", () => {
    ...
    test("headerCol + headerRow both apply header background", async () => {
      ...
      assert.ok(
        bgMatches && bgMatches.length === 3,
        `Expected 3 header-bg cells (corner + col header + row header), got ${bgMatches?.length}`,
      );
    });
  });
  ...
  describe("generatePreviewHTML nav bar", () => {
    test("single slide has dimmed prev/next arrows", () => {
      const pages = generatePreviewHTML([{ fragment: '<div class="root">slide</div>', label: "slide-1" }], mockTheme);
    ...
    test("multi-slide generates stable prev/next navigation", () => {
      const slides = [
        { fragment: "<div>s1</div>", label: "slide-1" },
        { fragment: "<div>s2</div>", label: "slide-2" },
        { fragment: "<div>s3</div>", label: "slide-3" },
      ];
    ...
    test("preview HTML has DOCTYPE and contains slide content", () => {
      const pages = generatePreviewHTML(
        [{ fragment: '<div class="root">SLIDE_CONTENT</div>', label: "slide-1" }],
        mockTheme,
      );
