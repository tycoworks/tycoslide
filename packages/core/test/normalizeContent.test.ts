...
describe("normalizeContent()", () => {
  ...
  test("mixed array: strings and NormalizedRuns", () => {
    const result = normalizeContent(["plain", { text: "bold", bold: true }]);
    assert.deepStrictEqual(result, [{ text: "plain" }, { text: "bold", bold: true }]);
  });
  ...
  test("TextRun with paragraphBreak property passes through", () => {
    const result = normalizeContent([{ text: "line1", paragraphBreak: true }, { text: "line2" }]);
    assert.deepStrictEqual(result, [{ text: "line1", paragraphBreak: true }, { text: "line2" }]);
  });
