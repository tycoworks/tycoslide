...
describe("buildTableCell()", () => {
  ...
  test("header column cell (not header row) uses headerBackground", () => {
    ...
    const result = builder.buildTableCell(
      ...
      2,
      0,
      1, // headerRows=0, headerColumns=1 (col 0 is header)
      tableNode,
    );
    const fill = result.options.fill as { color: string; transparency: number };
  ...
  test("header cell uses headerBackground from table style", () => {
    ...
    const result = builder.buildTableCell(
      ...
      1,
      1,
      0, // headerRows = 1, so row 0 is header
      tableNode,
    );
...
describe("buildTextFragments()", () => {
  test("converts string content to single fragment", () => {
    const fragments = builder.buildTextFragments("Plain text", mockTextStyle, "#000000");
  ...
  test("preserves color from text run", () => {
    const fragments = builder.buildTextFragments([{ text: "Colored", color: "#FF0000" }], mockTextStyle, "#000000");
  ...
  test("applies bold formatting", () => {
    const fragments = builder.buildTextFragments([{ text: "Bold text", bold: true }], mockTextStyle, "#000000");
  ...
  test("applies italic formatting", () => {
    const fragments = builder.buildTextFragments([{ text: "Italic text", italic: true }], mockTextStyle, "#000000");
  ...
  test("multiple paragraph breaks each get paraSpaceBefore", () => {
    const fragments = builder.buildTextFragments(
      [{ text: "Para 1" }, { text: "Para 2", paragraphBreak: true }, { text: "Para 3", paragraphBreak: true }],
      mockTextStyle,
      "#000000",
    );
  ...
  test("color override applies to all fragments", () => {
    const fragments = builder.buildTextFragments([{ text: "Run 1" }, { text: "Run 2" }], mockTextStyle, "#FF0000");
