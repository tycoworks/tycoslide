...
Layouts define the structure of a slide — where the title goes, where body content renders, and what fixed elements like footers appear. Each layout accepts specific frontmatter parameters and controls where content sits on the slide. This page covers the built-in layouts from `@tycoworks/tycoslide-theme` and how to create custom layouts for your own themes.

## Available Layouts

`@tycoworks/tycoslide-theme` provides 11 layouts. Custom themes can define their own (see [Creating Custom Layouts](#creating-custom-layouts)).
...
```typescript
import { defineLayout, param, token, SIZE, type InferTokens } from '@tycoworks/tycoslide';
import { textComponent, plainText, row, column } from '@tycoworks/tycoslide-components';
import type { PlainTextTokens } from '@tycoworks/tycoslide-components';
...
import { textComponent } from '@tycoworks/tycoslide-components';
...
Build layouts by composing container functions from `@tycoworks/tycoslide-components` to control how content is arranged on the slide:

```typescript
import { column, row, plainText } from '@tycoworks/tycoslide-components';
column(
...
The default theme's `title`, `section`, and `body` layouts in [`packages/theme-tycoworks/src/layouts.ts`](../packages/theme-tycoworks/src/layouts.ts) demonstrate all of these patterns — params, slots, masters, and composition with DSL functions.
...
```typescript
import { defineMaster, token, SIZE, VALIGN, Bounds, type InferTokens } from '@tycoworks/tycoslide';
import type { PlainTextTokens, SlideNumberTokens } from '@tycoworks/tycoslide-components';
import { row, column, plainText, slideNumber } from '@tycoworks/tycoslide-components';
...
```typescript
// my-theme/layouts.ts
import { defineLayout } from '@tycoworks/tycoslide';
...
```typescript
import { Presentation, layoutRegistry } from '@tycoworks/tycoslide';
import { theme, components, layouts } from '@tycoworks/tycoslide-theme';
import { componentRegistry } from '@tycoworks/tycoslide';
